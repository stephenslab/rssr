...
alt_rss_varbvsr_iter <- function(SiRiS,sigma_beta,logodds,betahat,se,alpha,mu,SiRiSr,reverse){
...
#   
  ...
  r0 <- alpha0 * mu0
  mu <- (sigma_square * (c(betahat) / se_square-  tSiRiS%*%(r0)))@x
  
  SSR <- mu0 * mu0 / sigma_square
