...
options <- list(plan=list(engine="MC",resources=list(nodes=2)),
                ...
                tolerance=1e-4)
datafiles <- tempfile()
mmat <- rss_varbvsr_parallel_future(datafiles,options=options)
test_that("We can estimate a known value of pi by grid search",
          expect_equal(c(marg_pi(log10oddsvec,c(mmat[[1]]))),(10/p),tolerance=0.01)
)
...
datafiles <- tempfile()
mmat <- rss_varbvsr_parallel_future(datafiles,options=options)
gsigb <- marg_param(c(mmat[[1]]),param = sigb)
