...
//' @param tmu0 a length p vector specifying the initial value of mu
//' @param SiRiSr0 a length p vector specifying the initial value of SiRiSr
double rss_varbvsr_squarem_iter_update(const c_Matrix_internal SiRiS,
                                const double sigma_beta,
                                const double logodds,
                                const c_arrayxd_internal  betahat,
                                const c_arrayxd_internal se,
...
Eigen::Array2d rss_varbvsr_squarem_alpha_iter(const c_Matrix_internal SiRiS,
                                        const double sigma_beta,
                                        const double logodds,
                                        const c_arrayxd_internal  betahat,
                                        const c_arrayxd_internal se,
  ...
  parallel_for(blocked_range<size_t>(0,tot_size),
               [&](const blocked_range<size_t>& r){
                 for(size_t t=r.begin(); t!=r.end(); t++){
                   ...
                   auto retvec=rss_varbvsr_squarem_iter(SiRiS,
                                                        sigma_beta(j),
                                                        logodds(i),
                                                        betahat,
                                                        se,
...
//                   npivec[t]=retvec[1];
  ...
  return(Rcpp::DataFrame::create(_["logodds"]=Rcpp::wrap(lovec),
                                 _["sigb"]=Rcpp::wrap(sigbvec),
  ...
  parallel_for(blocked_range<size_t>(0,tot_size),
               [&](const blocked_range<size_t>& r){
                 for(size_t t=r.begin(); t!=r.end(); t++){
                   ...
                   sigbvec(t)=sigma_beta(j);
                   lovec(t)=logodds(i);
                   auto retvec=rss_varbvsr_squarem_alpha_iter(SiRiS,
                                                              sigma_beta(j),
                                                              logodds(i),
                                                              betahat,
                                                              se,
...
Rcpp::List grid_rss_varbvsr_pve(
    const c_Matrix_internal SiRiS,
    const c_arrayxd_internal sigma_beta,
    const c_arrayxd_internal logodds,
    const c_arrayxd_internal  betahat,
  ...
  Eigen::ArrayXd salpha0=talpha0;
  Eigen::ArrayXd smu0=tmu0;
  Eigen::ArrayXd sSiRiSr0=tSiRiSr0;
  
  
  
  Eigen::MatrixXd alphamat(p,tot_size);
  Eigen::MatrixXd mumat(p,tot_size);
  ...
  Progress::Progress pp(tot_size, progress);
  for(size_t t=0; t<tot_size; t++){
    size_t i=t%logodds_size;
    size_t j=t/logodds_size;
    sigbvec(t)=sigma_beta(j);
    ...
    if (Progress::check_abort() ){
      return(Rcpp::List::create(_["alphamat"]=alphamat,
                                _["mumat"]=mumat,
                                _["grid_results"]=Rcpp::DataFrame::create(_["logodds"]=lovec,
                                                              _["sigb"]=sigbvec,
                                                              _["alpha_mean"]=npivec,
                                                              _["lnZ"]=nlzvec)));
    }
    ...
    pp.increment();
    double retlnz =rss_varbvsr_squarem_iter_update(SiRiS,
                                                sigma_beta(j),
                                                logodds(i),
                                                betahat,
                                                se,
                                                ...
                                                itermax,
                                                lnz_tol);
    alphamat.col(t)=salpha0;
    mumat.col(t)=smu0;
    nlzvec[t]=retlnz;
    npivec[t]=salpha0.mean();
  }
  return(Rcpp::List::create(_["alphamat"]=alphamat,
                            _["mumat"]=mumat,
                            _["grid_results"]=Rcpp::DataFrame::create(_["logodds"]=lovec,
